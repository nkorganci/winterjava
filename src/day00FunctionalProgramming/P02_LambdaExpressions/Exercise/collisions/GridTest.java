...
public class GridTest {
	...
	public static void main(String...args) {
		...
		// Warm up
		GridDetection.forEachCollision4(world, action);
		atomicCounter.reset();
		// Timed run
		Time.timeIt(() -> GridDetection.forEachCollision4(world, action));
		System.out.println("# of collisions: " + atomicCounter);
		
		
		System.out.println("\nTwo phases, collision stream + count");
		// Warm up
		long tempCounter = GridDetection.collisionStream(world).count();
		// Timed run
		Time.timeIt(() -> GridDetection.collisionStream(world).count());
